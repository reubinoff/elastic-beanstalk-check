...
class EnvironmentStatus:
    ...
    def __str__(self) -> str:
        return f"EnvironmentStatus(version_label={self.version_label}, status={self.status}, health_status={self.health_status})"
...
def main():
    ...
    while is_env_ready(env_status, app_version_label) is False and (time.time() - start) < timeout:
        time.sleep(5)
        label, status = get_environment_version(env_name, region)
    ...
    if is_env_ready(env_status, app_version_label) is False:
        print(f"Fail. Expected version {app_version_label} but got {label}. Status: {status}")
        sys.exit(1)
